...
def get_categoria(enlace):
    x=re.sub('(https://www.elvocero.com/ley-y-orden/)','',enlace)
    y=re.sub('/\S+','',x)
    return y.title()         
...
def get_noticias_vocero():
    ...
    enlaces = ['https://www.elvocero.com/ley-y-orden/federal/',
                'https://www.elvocero.com/ley-y-orden/policiacas/',
                'https://www.elvocero.com/ley-y-orden/justicia/']
    enlace=[]
    titulo=[]
    for link in enlaces:
    ...
    vocero = vocero.sort_values(by='fecha', ascending = False)
...
vocero = pd.read_csv('vocero_noticias.csv')
vocero['fecha'] = pd.to_datetime(vocero['fecha'])

antes=len(vocero)    
print("\nComenzo con {} articulos.".format(antes))
vocero_new=get_noticias_vocero()
vocero = pd.concat([vocero_new, vocero])
vocero = vocero.drop_duplicates(subset='enlace')

vocero.to_csv('vocero_noticias.csv',index=False)
despues = len(vocero)
